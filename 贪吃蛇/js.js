...
var change = false //记录方向是否改变，如果改变就改变事物的位置同时不再刷新食物
...
function move() { //小蛇如何行走
    ...
    switch (snake_length) {
        case 5:
            clearInterval(timer);
            timer = setInterval("move()", 0.9 * speed);
            break;
        case 7:
            clearInterval(timer);
            timer = setInterval("move()", 0.8 * speed);
            break;
        case 9:
            clearInterval(timer);
            timer = setInterval("move()", 0.7 * speed);
            break;
        case 11:
            clearInterval(timer);
            timer = setInterval("move()", 0.6 * speed);
            break;
        case 14:
            clearInterval(timer);
            timer = setInterval("move()", 0.5 * speed);
            break;
        case 17:
            clearInterval(timer);
            timer = setInterval("move()", 0.4 * speed);
            break;
        case 20:
            clearInterval(timer);
            timer = setInterval("move()", 0.3 * speed);
            break;
        case 22:
            clearInterval(timer);
            timer = setInterval("move()", 0.2 * speed);
            break;
        case 24:
            clearInterval(timer);
            timer = setInterval("move()", 0.1 * speed);
            break;
        case 25:
            clearInterval(timer);
            timer = setInterval("move()", 0.05 * speed);
            break;
    }
...
function keylinstener() {
    ...
    if (keycode == 37 && head != "right") {
        head = "left";
        console.log("left");
    }
    if (keycode == 38 && head != "down") {
        head = "up";
        console.log("up");
    }
    if (keycode == 39 && head != "left") {
        head = "right";
        console.log("right");
    }
    if (keycode == 40 && head != "up") {
        head = "down";
        console.log("down")
    }
    if (keycode == 32) {
...
function food() {
    ...
    map_position[food_x][food_y].style.backgroundColor = "#d6d8d7" //rgb(71, 174, 206)
    console.log(food_x)
    console.log(food_y)
}
food()
